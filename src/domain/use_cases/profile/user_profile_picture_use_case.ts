...
export async function getProfilePictureUseCase(userId: string) {
    const authenticationReposiory = getServerInjection("IFirebaseRepository");
    const path = "user_profiles/" + userId + ".png";
    return await authenticationReposiory.getUrlFromDocumentPath(path);
}

export async function updateProfilePictureUseCase(userId: string, file: File) {
    ...
    const authenticationReposiory = getServerInjection("IFirebaseRepository");
    const path = "user_profiles/" + userId + ".png";
    return await authenticationReposiory.uploadFile(path, file);
}