...
export async function removeSubstepAdvancementUsecase(examId: string, stepId: string, substepId: string) {
    ...
    if (session)
    {
        ...
        // remove substepId from readSubsteps
        advancement.readSubsteps.splice(advancement.readSubsteps.indexOf(substepId), 1);
